...
export class ContractServices {
  ...
  constructor() {
    if (typeof window.ethereum !== "undefined") {
      ...
      this.contract = new this.web3.eth.Contract(
        this.contractABI as AbiInput[],
        this.contractAddress
      );
      this.coinContract = new this.web3.eth.Contract(
        this.coinContractABI as AbiInput[],
        this.MytokenAddress
      );
  ...
  async getAmountMytoken() {
    this.contractBalance.next(
      await this.coinContract.methods.balanceOf(this.contractAddress).call()
    );
  }

  //  conect to smart contracto to pay trasaction
  payTransaction(amount: number): Promise<void> {
